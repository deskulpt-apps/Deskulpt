...
pub struct Widgets(BTreeMap<String, Widget>);

impl Widgets {
    pub fn get(&self, id: &str) -> Result<&Widget> {
        self.0
            .get(id)
            .ok_or_else(|| anyhow!("Widget not found: {id}"))
    }

    pub fn get_mut(&mut self, id: &str) -> Result<&mut Widget> {
        self.0
            .get_mut(id)
            .ok_or_else(|| anyhow!("Widget not found: {id}"))
    }

    pub fn iter(&self) -> impl Iterator<Item = (&String, &Widget)> {
        self.0.iter()
    }

    pub fn values(&self) -> impl Iterator<Item = &Widget> {
        self.0.values()
    }

    pub fn insert_or_remove(&mut self, id: String, widget: Option<Widget>) -> bool {
        match widget {
            Some(w) => {
                self.0.insert(id, w);
                true
    ...
    // pub fn insert(&mut self, id: String, widget: Widget) -> Option<Widget>
    // {     self.0.insert(id, widget)
    // }

    // pub fn remove(&mut self, id: &str) -> Option<Widget> {
    //     self.0.remove(id)
    // }
}
