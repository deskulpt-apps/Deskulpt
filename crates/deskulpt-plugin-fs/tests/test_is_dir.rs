...
fn test_is_dir_for_directory() {
    ...
    assert!(result.is_ok());
    let is_dir: bool = serde_json::from_value(result.unwrap()).unwrap();
    assert!(is_dir);
}

#[test]
fn test_is_dir_for_file() {
    let temp_dir = TempDir::new().unwrap();
...
#[test]
fn test_is_dir_for_nonexistent() {
    let temp_dir = TempDir::new().unwrap();
    let plugin = FsPlugin;
    let widget_dir_fn = create_widget_dir_fn(temp_dir.path().to_path_buf());
...
#[test]
fn test_is_dir_nested() {
    let temp_dir = TempDir::new().unwrap();
    std::fs::create_dir_all(temp_dir.path().join("nested").join("deep")).unwrap();
